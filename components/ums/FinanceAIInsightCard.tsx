...
export function FinanceAIInsightCard({ insight, onDismiss, onAction }: FinanceAIInsightCardProps) {
  ...
  return (
    <MotionCard className="ai-severity-card flex flex-col gap-3 p-4 rounded-[10px] border-l-4" style={{ borderColor: config.color, backgroundColor: config.bg }}>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-[600] uppercase tracking-wide" style={{ color: config.color }}>
              {insight.severity}
            </p>
        ...
        <button
          onClick={onDismiss}
          className="p-1 hover:bg-white rounded-[6px] transition-colors flex-shrink-0"
        >
      ...
      <button
        ...
        className="flex items-center justify-between gap-2 py-1.5 -mx-2 px-2 hover:bg-white/30 rounded-[6px] transition-colors"
      >
